...
local token = '364959406:AAGMv3jsjguP_UVsI2-laPnoH0-houfyj90'  --[[Enter tokn here]]
...
function is_mod(chat,user)
	sudo = {194279067,425998008,000000000,000000000,000000000}  --[[Enter your id and cli bot id here]]
 local var = false
  ...
  while true do
    ...
    if(updates) then
      if (updates.result) then
		...
		if q.from.id == 425998008 or q.from.id == 194279067 then
 ...
 if redis:get(hash) then
    ...
    return false
              ...
              local keyboard = {}
				 ...
				 {text = 'پشتیبانی ربات ', callback_data = 'supportbot:'..chat},{text = 'تبلیغات شما', callback_data = 'youradds:'..chat}
				  },{
				 {text = 'اطلاعات گروه', callback_data = 'groupinfo:'..chat},{text = 'راهنما استفاده', callback_data = 'helpbot:'..chat}
				}
							}
            answer(q.id,'settings','Group settings',chat,'به بخش اصلی ربات اورین خوش آمدید.\nاز منوی زیر انتخاب کنید:',keyboard)
            end
            end
						end
          if msg.callback_query then
            local q = msg.callback_query
             ...
             if q.data:match('_') and not (q.data:match('next_page') or q.data:match('left_page')) then
                Canswer(q.id,">برای مشاهده راهنمای بیشتر این بخش عبارت\n/help\nرا ارسال کنید\n>تیم پشتیبانی:[@yenial_bot]\n>ربات خرید :[@avrin_team]\n>کانال ربات اورین:[@yenial_admin]",true)
...
local hash = SUDO..'settings:'..chat..':'..value
 if redis:get(hash) then
    ...
    return false
            ...
            edit(q.inline_message_id,'`به بخش اصلی ربات هوشمند اورین خوش آمدید /nیکی از زیر منوها را انتخاب کنید:`',keyboard)
            end
							------------------------------------------------------------------------
		...
		local keyboard = {}
              ...
              edit(q.inline_message_id,'`شرکت یِنی آل تبلیغات شما را به صورت خرده و انبوه میپذیرد برای سفارش تبلیغات در کانال و گروه ها به ایدی /n@yenial_admin`',keyboard)
            end
							------------------------------------------------------------------------
       ...
       if h == -1 then
        ...
        elseif is_lock(chat,value) then
          ...
          return '🔓'
            ...
            else Canswer(q.id,'شما مدیر یا مالک گروه نیستید!/nبرای خرید ربات هوشمند اورین به ایدی @yenial_admin پیام دهید/nکانال ما:@avrin_team',true)
						end
						end
